//6859b93137624e319d6202133240912


async function search(sear) {
    let x = await fetch(`https://api.weatherapi.com/v1/forecast.json?key=6859b93137624e319d6202133240912&q=cairo&days=3`);
    ...
    if (null != x) {
        ...
        let cartona = `<div class="today forecast ">
                    <div class="forecast-header d-flex justify-content-between" id="today">
                        ...
                        <div class="date text-light">${e.getDate() + monthNames[e.getMonth()]}</div>
                    </div>
                    <div class="forecast-content" id="current">
                       <div class="location text-light">${finalResult.name}</div>
                        <div class="degree">
                            ...
                            <div class="forecast-icon">
                                <img src="https:${x.condition.icon}" alt width="90">
                            </div>
                        </div>
                        <div class="custom">${x.condition.text}</div>
                        <span class="ms-3 text-light"><img src="./icon-umberella.png" class="me-1" alt="umberalla">${x.humidity}%</span>
                        <span class="ms-3 text-light"><img src="./icon-wind.png" class="me-1" alt="wind">${x.wind_kph}</span>
                        
                        <span class="ms-3 text-light"><img src="./icon-compass.png" class="me-1" alt="compass">${x.wind_dir}</span>
                    </div>  
                </div>`
...
function display2(finalResult) {
    ...
    for (let i = 1; i < finalResult.length; i++)
        cartona += `  <div class="forecast c text-center">
                    <div class="forecast-header ">
                        <div class="day text-light">${day[new Date(finalResult[i].date.replace(" ", "T")).getDay()]}</div>
                    </div>
                    <div class="forecast-content">
                     </div> \x3c!-- .forecast-header --\x3e\n

                        <div class="forecast-icon">
                            <img src="https:${finalResult[i].day.condition.icon}" alt width="48">
                        </div>
                        ...
                        <div class="custom">${finalResult[i].day.condition.text} </div>
                    </div>
                `
